  ...
  try {
    ...
    const lightAudit = await page.evaluate(() => {
      ...
      return {
        ...
        profileSnow: getStyle(profileSnow),
        catSnowCount: catSnows.filter(Boolean).length,
        catSnows: catSnows.map(getStyle),
        postCount: postCards.length,
        postSnowCount: postSnows.filter(Boolean).length,
        postSnows: postSnows.slice(0, 8).map(getStyle),
        asideCount: asideCards.length,
        asideSnowCount: asideSnows.filter(Boolean).length,
      };
    });

    console.log('Light Mode Snow Audit Results:');
    console.log('- Canvas Snow Universe Active:', lightAudit.canvasExists, 'Opacity:', lightAudit.canvasOpacity);
    console.log('- TodayCard Snow Cover:', lightAudit.todaySnow?.display, 'height:', lightAudit.todaySnow?.height);
    console.log('- RandomBanner Snow Cover:', lightAudit.bannerSnow?.display, 'height:', lightAudit.bannerSnow?.height);
    console.log('- NoticeBar Snow Cover:', lightAudit.noticeSnow?.display, 'height:', lightAudit.noticeSnow?.height);
    console.log('- Category Items with Snow:', `${lightAudit.catSnowCount} items`);
    console.log('- Post Cards with Snow:', `${lightAudit.postSnowCount} / ${lightAudit.postCount} cards`);
    console.log('- Aside Widgets with Snow:', `${lightAudit.asideSnowCount} / ${lightAudit.asideCount} widgets`);

    if (!lightAudit.canvasExists || lightAudit.canvasOpacity !== '1') {
      throw new Error('FAIL: Canvas snow universe is not active');
    }
    if (lightAudit.todaySnow?.display !== 'block') {
      throw new Error('FAIL: Today card snow cover is missing or not visible');
    }
    if (lightAudit.postSnowCount < 4) {
      throw new Error(`FAIL: Post cards missing snow covers (found ${lightAudit.postSnowCount})`);
    }

    // Verify diversity of category buttons
    const catImages = lightAudit.catSnows.map(c => c?.backgroundImage);
    ...
    // Verify diversity of post stream cards
    const postImages = lightAudit.postSnows.map(p => p?.backgroundImage);
    ...
    const darkAudit = await page.evaluate(() => {
      ...
      const csSnow = todaySnow ? window.getComputedStyle(todaySnow) : null;

      return {
        theme: document.documentElement.dataset.theme,
        canvasOpacity: csCanvas?.opacity,
        snowFilter: csSnow?.filter,
        snowBg: csSnow?.backgroundImage?.slice(0, 60),
      };
    });
    ...
    if (!darkAudit.snowFilter || darkAudit.snowFilter === 'none') {
      throw new Error('FAIL: Dark mode snow cover is missing moonlit drop-shadow glow filter');
    }
    console.log('✅ Dark mode moonlit snow cover verified with atmospheric glow!');
    ...
    if (bgBtn) {
      ...
      if (cleanCheck.snowDisplay !== 'none') {
        throw new Error('FAIL: Snow cover was still displayed in clean mode');
      }
      console.log('✅ Clean mode clean baseline confirmed: all snow covers and canvas suspended!');
      ...
      const snowRestoreCheck = await page.evaluate(() => {
        ...
        return {
          ...
          canvasOpacity: csCanvas?.opacity,
          snowDisplay: csSnow?.display,
          snowBg: csSnow?.backgroundImage?.slice(0, 40),
        };
      });
      ...
      if (snowRestoreCheck.snowDisplay !== 'block') {
        throw new Error('FAIL: Snow cover did not restore after switching back to snow');
      }
      console.log('✅ Snow mode successfully restored with active snow universe and card covers!');
    }
