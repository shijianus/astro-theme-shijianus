...
async function run() {
  ...
  await page.goto('http://localhost:4321/archives/', { waitUntil: 'networkidle' });

  const analysis = await page.evaluate(() => {
    const widgets = Array.from(document.querySelectorAll('#aside-content .card-widget'));
  ...
  await page.screenshot({ path: 'scripts/archives_layout.png', fullPage: false });
  await browser.close();
}
