---
title: "Laboratorio de Control de Acceso a Artículos"
pubDate: 2026-04-26
...
coverAlt: "Laboratorio de Control de Acceso a Artículos"
featured: false
sticky: 1
tags: ["Control de Acceso", "Seguridad", "Renderizado del Lado del Servidor"]
access:
  passwordHash: "5994471abb01112afcc18159f6cc74b4f511b99806da59b3caf5a9c173cacfc5"
  message: "Este artículo tiene habilitado el control de acceso del lado del servidor. El contenido solo se renderizará después de introducir la contraseña correcta."
...
1.  El contenido principal no aparece en el HTML cuando no se ha introducido la contraseña.
2.  Después de introducir la contraseña correcta `12345`, el servidor escribirá una credencial de acceso temporal.
3.  Al recargar este artículo, no será necesario volver a introducir la contraseña.
...
-   Visible para IPs específicas
-   Invisible para IPs específicas
...
Aunque actualmente sigue siendo compatible escribir directamente `password`, se recomienda más escribir solo `passwordHash` en Markdown. De esta manera, el tema solo realizará la comparación en el lado del servidor, sin necesidad de guardar la contraseña en texto plano en la configuración del contenido.

Si necesitas generar tu propio valor hash, el tema actual utiliza `SHA-256` internamente. Se recomienda convertir la contraseña a hash localmente y luego escribirla en el frontmatter, en lugar de colocar la contraseña en texto plano directamente en el archivo fuente del artículo.

## Por Qué Esta Capa No Filtra el Contenido Restringido de Antemano

Esta implementación no es "primero generar el texto completo y luego ocultarlo con el frontend". Las páginas de artículos restringidos se basan en la determinación del lado del servidor:

1.  Cuando no se cumplen las reglas de contraseña o de región/IP, el servidor solo devuelve el panel de bloqueo.
2.  El contenido principal, el índice, los artículos relacionados y los resúmenes públicos no se renderizarán en la página si no está desbloqueado.
