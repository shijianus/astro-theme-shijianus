...
- [x] 附录与名片机制说明：名片浮层仅陈列权重最高的前 4 个已解锁徽章（拒绝花哨冗余），并由“最新发言 · 加入时间 · 已读时长 · 喝彩”横向弹性流精准排印；
- [x] 编写并执行全流程自动化端到端测试套件（`scripts/verify-badges-guide.mjs`），全量验证文章标题、封面、标签、15 枚徽章编号、25 个 TOC 锚点、33 个 Callout 动态卡片与信任阶梯逻辑断言全绿通过。
