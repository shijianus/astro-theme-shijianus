...
- [x] **全流程自动化测试与构建验证**：
  - 编写并执行 `scripts/verify-ai-summary-levels.mjs`，全量验证 low、medium、high 档位正文裁剪与字数上限、完整高档位 Prompt 与模型池机制（断言 100% PASS）；
  - 编写并执行 `scripts/verify-ai-interactions.mjs`，在真实浏览器中验证高档位摘要与各按键交互无错通过；
  - 执行 `npm run build`，99 个页面构建完成，0 报错。
